...
def plot_anova_groups(df: pd.DataFrame) -> Tuple[go.Figure, float]:
    ...
    fig.update_layout(title=f'<b>ANOVA:</b> Comparing Reagent Lot Performance', yaxis_title='Library Yield (ng/µL)', plot_bgcolor='white', showlegend=False); return fig, p_val
def plot_permutation_test(df: pd.DataFrame, n_permutations: int = 1000) -> go.Figure:
    ...
    g1_data = df[df['Reagent_Lot'] == groups[0]]['Library_Yield'].dropna(); g2_data = df[df['Reagent_Lot'] == groups[1]]['Library_Yield'].dropna(); observed_diff = g1_data.mean() - g2_data.mean(); concat_data = np.concatenate([g1_data, g2_data]); perm_diffs = []
...
def plot_doe_cube(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_traces(lines); fig.update_layout(title="<b>DOE:</b> Design Space", scene=dict(xaxis_title='A: Primer Conc', yaxis_title='B: Anneal Temp', zaxis_title='C: PCR Cycles'), margin=dict(l=0, r=0, b=0, t=40)); return fig
def plot_doe_effects(df: pd.DataFrame) -> Tuple[go.Figure, go.Figure]:
    main_effects = {f: df.loc[df[f] == 1, 'Library_Yield'].mean() - df.loc[df[f] == -1, 'Library_Yield'].mean() for f in ['Primer_Conc', 'Anneal_Temp', 'PCR_Cycles']}; fig_main = px.bar(x=list(main_effects.keys()), y=list(main_effects.values()), color=list(main_effects.keys()), color_discrete_map={'Primer_Conc': COLORS['primary'], 'Anneal_Temp': COLORS['accent'], 'PCR_Cycles': COLORS['secondary']}, labels={'x': 'Factor', 'y': 'Effect on Library Yield (ng)'}, title="<b>DOE: Main Effects</b>"); fig_main.update_layout(plot_bgcolor='white', showlegend=False); fig_int = go.Figure()
    ...
    fig_int.update_layout(title="<b>DOE: Interaction (Temp*Cycles)</b>", xaxis_title="Factor C: PCR Cycles", yaxis_title="Mean Library Yield (ng)", plot_bgcolor='white', legend_title_text='Factor B Level'); return fig_main, fig_int
def plot_bayesian_optimization_interactive(true_func, x_range: np.ndarray, sampled_points: Dict[str, list]) -> Tuple[go.Figure, float]:
    X_sampled, y_sampled = np.array(sampled_points['x']).reshape(-1, 1), np.array(sampled_points['y']); kernel = C(1.0, (1e-3, 1e3)) * RBF(10, (1e-2, 1e2)); alpha = np.std(y_sampled)**2 if len(y_sampled) > 1 else 0.1**2
    gp = GaussianProcessRegressor(kernel=kernel, n_restarts_optimizer=10, alpha=alpha, normalize_y=True); gp.fit(X_sampled, y_sampled); y_mean, y_std = gp.predict(x_range.reshape(-1, 1), return_std=True); ucb = y_mean + 1.96 * y_std; next_point_x = x_range[np.argmax(ucb)]; fig = go.Figure()
    ...
    fig.update_layout(title_text="<b>Bayesian Optimization:</b> Smart Search for Optimal Conditions", xaxis_title="Parameter Setting (e.g., Enzyme Concentration)", yaxis_title="Assay Performance (e.g., On-Target %)", plot_bgcolor='white', legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)); return fig, next_point_x
...
def plot_rul_prediction(df: pd.DataFrame) -> go.Figure:
    time, signal = df['Run_Number'].values, df['Laser_Power_mW'].values; threshold, current_time = 80.0, 70; model_time, model_signal = time[time < current_time], signal[time < current_time]; p = np.polyfit(model_time, np.log(model_signal), 1); future_time = np.arange(current_time, 120); pred_signal = np.exp(p[1]) * np.exp(p[0] * future_time); failure_indices = np.where(pred_signal <= threshold)[0]
...
def plot_ewma_chart(df: pd.DataFrame, lambda_val: float = 0.2) -> go.Figure:
    mean, std_dev = df['Yield_ng'].iloc[:75].mean(), df['Yield_ng'].iloc[:75].std(ddof=1); df['ewma'] = df['Yield_ng'].ewm(span=(2 / lambda_val) - 1).mean(); n = np.arange(1, len(df) + 1); ucl_ewma = mean + 3 * std_dev * np.sqrt(lambda_val / (2 - lambda_val) * (1 - (1 - lambda_val) ** (2 * n))); lcl_ewma = mean - 3 * std_dev * np.sqrt(lambda_val / (2 - lambda_val) * (1 - (1 - lambda_val) ** (2 * n)))
    ...
    fig.update_layout(title=f'<b>EWMA Chart (λ={lambda_val}):</b> Detecting Small Drifts', xaxis_title='Batch ID', yaxis_title='Yield (ng)', plot_bgcolor='white'); return fig
