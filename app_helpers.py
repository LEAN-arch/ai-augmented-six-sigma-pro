...
def plot_gage_rr_variance_components() -> go.Figure:
    """Creates a stacked bar chart of variance components for Gage R&R."""
    data = {
        'Source': ['% Contribution'],
        'Part-to-Part': [85],
        'Gage R&R': [15],
    ...
    fig.update_layout(
        barmode='stack',
        title='<b>Gage R&R:</b> Sources of Variation',
        xaxis_title='Percentage of Total Variation',
...
def plot_process_mining_graph() -> graphviz.Digraph:
    """Creates a process mining visualization."""
    ...
    dot.node('A', 'Order Received')
    dot.node('B', 'Check Inventory')
    dot.node('C', 'Ship Product')
    dot.node('D', 'Invoice Sent')
    ...
    # Edges with weights and labels
    dot.edge('start', 'A', label='  15 Cases')
    dot.edge('A', 'B', label='  15 Cases')
...
def plot_vsm() -> go.Figure:
    """Creates a simplified Value Stream Map visualization."""
...
def plot_capability_analysis_pro(data: np.ndarray, lsl: float, usl: float) -> Tuple[go.Figure, float, float]:
    """Creates a comprehensive capability plot with histogram and KDE."""
...
def plot_fishbone_diagram() -> graphviz.Digraph:
    ...
    dot = graphviz.Digraph(engine='neato', graph_attr={'splines': 'line'}) # neato is better for this layout
    
    # Effect
    dot.node('Effect', 'Low Product Yield', shape='box', style='filled', fillcolor=f"{COLORS['accent']}50")
    
    # Main bones
    cats = ['Methods', 'Machines', 'Manpower', 'Materials', 'Measurement', 'Environment']
    positions = [(0, 2), (0, -2), (3, 2), (3, -2), (6, 2), (6, -2)]
    for i, cat in enumerate(cats):
...
def plot_pareto_chart() -> go.Figure:
    ...
    fig.update_layout(
        title_text="<b>Pareto Chart:</b> Identifying the 'Vital Few' Defects",
...
def plot_anova_groups(df: pd.DataFrame) -> Tuple[go.Figure, float]:
    """Creates a box plot for ANOVA and calculates p-value."""
...
def plot_permutation_test(df: pd.DataFrame, n_permutations: int = 1000) -> go.Figure:
    """Performs and plots a permutation test for difference in means."""
    groups = df['Supplier'].unique()
    group_data = [df[df['Supplier'] == g]['Yield'] for g in groups]
    ...
    fig.update_layout(
        ...
        xaxis_title=f'Difference in Mean Yield ({groups[0]} - {groups[1]})',
...
def plot_regression_comparison_pro(df: pd.DataFrame) -> Tuple[go.Figure, RandomForestRegressor, pd.DataFrame]:
    """Compares Linear and Random Forest regression models."""
...
def plot_shap_summary(model: RandomForestRegressor, X: pd.DataFrame) -> go.Figure:
    ...
    for i, feature in enumerate(X.columns):
        y_vals = np.full(X.shape[0], i)
        # Add a bit of random jitter for beeswarm effect
        y_vals = y_vals + np.random.uniform(-0.2, 0.2, X.shape[0])
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                color=X[feature],
                colorscale='RdBu',
                showscale=True if i == 0 else False, # Show scale only for the first trace
                colorbar=dict(title="Feature Value", x=-0.3),
                cmin=X[feature].min(),
                cmax=X[feature].max()
            ),
            showlegend=False,
            hovertext=[f"{feature}={val:.2f}" for val in X[feature]],
            hoverinfo='text'
        ))

    fig.update_layout(
        title="<b>SHAP Summary Plot:</b> Feature Impact on Yield Prediction",
...
def plot_doe_cube(df: pd.DataFrame) -> go.Figure:
    """Creates a 3D cube plot for a 2^3 DOE."""
    fig = go.Figure(data=[go.Scatter3d(
        x=df['Temp'], y=df['Pressure'], z=df['Time'], mode='markers+text',
    ...
    fig.update_layout(
        title="<b>DOE:</b> 2³ Factorial Design Cube Plot",
...
def plot_doe_effects(df: pd.DataFrame) -> Tuple[go.Figure, go.Figure]:
    """Calculates and plots main effects and interaction plots for a 2-level DOE."""
    # Main Effects
    main_effects = {}
    for factor in ['Temp', 'Pressure', 'Time']:
        effect = df.loc[df[factor] == 1, 'Yield'].mean() - df.loc[df[factor] == -1, 'Yield'].mean()
        main_effects[factor] = effect
    
    # Main Effects Plot
    fig_main = go.Figure(go.Bar(
        x=list(main_effects.keys()), y=list(main_effects.values()),
        marker_color=[COLORS['primary'], COLORS['accent'], COLORS['secondary']]
    ))
    fig_main.update_layout(
        title="<b>DOE:</b> Main Effects Plot",
        xaxis_title="Factor", yaxis_title="Effect on Yield",
        plot_bgcolor='white', paper_bgcolor='white'
    )
    ...
    fig_int.update_layout(
        title="<b>DOE:</b> Interaction Plot (Temp*Pressure)",
        xaxis_title="Factor: Temperature", yaxis_title="Mean Yield",
        plot_bgcolor='white', paper_bgcolor='white',
...
def plot_bayesian_optimization_interactive(true_func, x_range: np.ndarray, sampled_points: Dict[str, list]) -> Tuple[go.Figure, float]:
    """Plots the interactive Bayesian Optimization process."""
    ...
    y_std = np.nan_to_num(y_std, 0) # Handle potential numerical issues
...
def plot_fmea_table() -> go.Figure:
    """Creates a color-coded table for FMEA results."""
    ...
    fig = go.Figure(data=[go.Table(
        ...
        cells=dict(
            ...
            fill_color=[['#FFFFFF']*len(df)]*3 + [colors],
            align='left'
        )
    )])
    fig.update_layout(
        title="<b>FMEA:</b> Risk Priority Number (RPN) Analysis",
        margin=dict(l=10, r=10, t=50, b=10)
    )
    return fig

def plot_rul_prediction(df: pd.DataFrame) -> go.Figure:
    """Creates a Remaining Useful Life prediction plot."""
    ...
    try:
        ...
        time_to_failure = future_time[failure_idx] - current_time
        rul_text = f"Predicted RUL: {time_to_failure:.1f} Hours"
    except IndexError:
        time_to_failure = np.inf
        rul_text = "Predicted RUL: > 50 Hours"
    ...
    fig.add_vrect(x0=current_time, x1=current_time + (time_to_failure if time_to_failure != np.inf else 50), fillcolor=COLORS['secondary'], opacity=0.2, line_width=0, name='RUL Window')
    ...
    fig.update_layout(
        title='<b>PHM:</b> Predicting Remaining Useful Life (RUL)',
        xaxis_title='Operating Time (Hours)', yaxis_title='Vibration Signal (g)',
        plot_bgcolor='white', paper_bgcolor='white',
...
def plot_shewhart_chart(df: pd.DataFrame) -> go.Figure:
    """Creates a basic Shewhart (X-bar) control chart."""
    mean = df['Value'].iloc[:75].mean()
    std_dev = df['Value'].iloc[:75].std()
...
def plot_control_plan() -> go.Figure:
    """Creates a table visualization of a Control Plan."""
    data = {
        'Process Step': ['Curing', 'Curing', 'Final Assembly'],
        'Characteristic (X or Y)': ['Oven Temperature (X)', 'Cure Time (Y)', 'Torque on Bolt A (X)'],
        'Specification': ['150 ± 5 °C', '60 ± 2 min', '25 ± 1 Nm'],
        'Tool': ['Thermocouple', 'Timer', 'Torque Wrench'],
        'Method': ['EWMA Chart', 'X-bar Chart', 'SPC Chart'],
        'Sample Size': [1, 5, 1],
        'Frequency': ['Continuous', 'Per Batch', 'Per Unit'],
        'Reaction Plan': ['Halt & Adjust', 'Investigate Batch', 'Recalibrate Wrench']
    }
    df = pd.DataFrame(data)
    fig = go.Figure(data=[go.Table(
        header=dict(values=list(df.columns), fill_color=COLORS['dark_gray'], font=dict(color='white'), align='left', height=40),
        cells=dict(values=[df[col] for col in df.columns], align='left', height=30)
    )])
    fig.update_layout(
        title="<b>Control Plan:</b> Formalizing Process Controls",
        margin=dict(l=10, r=10, t=50, b=10)
    )
    return fig
