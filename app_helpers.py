...
from scipy.stats import norm, gaussian_kde, f, chi2_contingency
import graphviz
...
def generate_process_data(mean, std_dev, size, lsl, usl):
    ...
    if num_outliers > 0:
        outliers = np.concatenate([np.random.uniform(lsl - 3 * std_dev, lsl, num_outliers // 2), np.random.uniform(usl, usl + 3 * std_dev, num_outliers - (num_outliers // 2))]);
        data[:len(outliers)] = outliers
    np.random.shuffle(data); return data

def generate_nonlinear_data(size=200):
    X1 = np.linspace(-10, 10, size); X2 = X1**2; X3 = np.random.randn(size) * 5; y = 2 * X1 + 0.5 * X2 + np.random.normal(0, 5, size)
...
def generate_control_chart_data(mean=100, std_dev=5, size=150, shift_point=100, shift_magnitude=1.5):
    in_control = np.random.normal(mean, std_dev, shift_point); out_of_control = np.random.normal(mean + shift_magnitude * std_dev, std_dev, size - shift_point)
...
def generate_kano_data():
    np.random.seed(42)
    # Basic (Must-be)
    basic = pd.DataFrame({'functionality': np.linspace(0, 10, 20), 'satisfaction': np.linspace(-8, 0, 20) + np.random.normal(0, 0.5, 20)})
    ...
    excitement = pd.DataFrame({'functionality': np.linspace(0, 10, 20), 'satisfaction': np.linspace(0, 8, 20) + np.random.normal(0, 0.5, 20)})
    excitement['category'] = 'Excitement (Delighter)'
    return pd.concat([basic, performance, excitement], ignore_index=True)

def generate_process_log_data():
    return pd.DataFrame([
        {'case_id': 1, 'activity': 'Order Received', 'timestamp': '2023-01-01 09:00'}, {'case_id': 1, 'activity': 'Check Inventory', 'timestamp': '2023-01-01 09:05'}, {'case_id': 1, 'activity': 'Ship Product', 'timestamp': '2023-01-01 14:00'}, {'case_id': 1, 'activity': 'Invoice Sent', 'timestamp': '2023-01-01 14:15'},
        {'case_id': 2, 'activity': 'Order Received', 'timestamp': '2023-01-01 09:30'}, {'case_id': 2, 'activity': 'Check Inventory', 'timestamp': '2023-01-01 09:35'}, {'case_id': 2, 'activity': 'Request Restock', 'timestamp': '2023-01-01 09:40'}, {'case_id': 2, 'activity': 'Wait for Stock', 'timestamp': '2023-01-02 11:00'}, {'case_id': 2, 'activity': 'Ship Product', 'timestamp': '2023-01-02 11:15'}, {'case_id': 2, 'activity': 'Invoice Sent', 'timestamp': '2023-01-02 11:20'},
        {'case_id': 3, 'activity': 'Order Received', 'timestamp': '2023-01-01 10:00'}, {'case_id': 3, 'activity': 'Check Inventory', 'timestamp': '2023-01-01 10:05'}, {'case_id': 3, 'activity': 'Ship Product', 'timestamp': '2023-01-01 15:00'}, {'case_id': 3, 'activity': 'Invoice Sent', 'timestamp': '2023-01-01 15:15'},
    ] * 5) # Repeat to make graph thicker
...
def generate_sensor_degradation_data():
    ...
    # Sensor degrades linearly with some noise, then fails
    degradation = 0.01 * time**2 + np.random.normal(0, 0.5, 100)
    degradation[85:] += np.random.normal(5, 2, 15) # Accelerate failure
...
def plot_kano_model():
    ...
    fig = go.Figure()
    for cat, color in zip(df['category'].unique(), [COLORS['accent'], COLORS['primary'], COLORS['secondary']]):
        subset = df[df['category'] == cat]
        fig.add_trace(go.Scatter(x=subset['functionality'], y=subset['satisfaction'], mode='markers', name=cat, marker_color=color))
    fig.add_hline(y=0, line_width=1, line_color='black')
    fig.add_vline(x=5, line_width=1, line_color='black', line_dash='dash')
    fig.update_layout(title='<b>Kano Model:</b> Prioritizing Customer Requirements', xaxis_title='Degree of Functionality (Provided vs. Not Provided)', yaxis_title='Customer Satisfaction (Dissatisfied vs. Delighted)', plot_bgcolor='white', paper_bgcolor='white', legend=dict(x=0.01, y=0.99))
    return fig

def plot_process_mining_graph():
    dot = graphviz.Digraph(comment='Process Mining')
    dot.attr('node', shape='box', style='rounded,filled', fillcolor='lightblue')
    dot.attr('edge', color='gray40')
    dot.node('start', 'Start', shape='circle', fillcolor='green')
    dot.node('end', 'End', shape='doublecircle', fillcolor='orange')
    dot.edge('start', 'Order Received', label='15')
    dot.edge('Order Received', 'Check Inventory', label='15')
    dot.edge('Check Inventory', 'Ship Product', label='10', penwidth='3.0')
    dot.edge('Ship Product', 'Invoice Sent', label='15', penwidth='3.5')
    dot.edge('Invoice Sent', 'end', label='15')
    dot.edge('Check Inventory', 'Request Restock', label='5', color='red', penwidth='1.5')
    dot.edge('Request Restock', 'Wait for Stock', label='5', color='red', penwidth='1.5')
    dot.edge('Wait for Stock', 'Ship Product', label='5', color='red', penwidth='1.5')
    return dot

def plot_anova_groups(df):
    groups = df['Supplier'].unique()
    fig = go.Figure()
    for group, color in zip(groups, [COLORS['primary'], COLORS['secondary'], COLORS['accent']]):
        fig.add_trace(go.Box(y=df[df['Supplier'] == group]['Yield'], name=group, marker_color=color))
    
    # Perform ANOVA
    f_val, p_val = f(*[df[df['Supplier'] == g]['Yield'] for g in groups])
...
def plot_rul_prediction(df):
    time = df['Time'].values
    signal = df['Vibration_Signal'].values
    
    # Simple ML model: linear fit on early data, extrapolate to failure threshold
    threshold = 12.0
    model = LinearRegression()
    early_time = time[:50].reshape(-1, 1)
    early_signal = signal[:50]
    model.fit(early_time, early_signal)
    
    # Predict over full range
    full_pred = model.predict(time.reshape(-1, 1))
    
    # Calculate RUL from a certain point, e.g., T=60
    current_time = 60
    current_pred_signal = model.predict(np.array([[current_time]]))[0]
    time_to_failure = (threshold - current_pred_signal) / model.coef_[0]
...
# --- Keep existing plotting functions ---
