...
# ==============================================================================
# This section remains unchanged as the data generation logic is sound.
...
def generate_nonlinear_data(size: int = 200) -> pd.DataFrame:
    np.random.seed(42)
    X1 = np.linspace(55, 65, size); X2 = 1.0 + 0.1 * (X1 - 60)**2 + np.random.normal(0, 0.5, size); X3_noise = np.random.randn(size) * 5
...
def generate_anova_data(means: list, stds: list, n: int) -> pd.DataFrame:
    ...
    for i, (mean, std) in enumerate(zip(means, stds)):
        data.extend(np.random.normal(mean, std, n)); groups.extend([f'Lot {chr(65+i)}'] * n)
    return pd.DataFrame({'Library_Yield': data, 'Reagent_Lot': groups})
def generate_sensor_degradation_data() -> pd.DataFrame:
    np.random.seed(42); time = np.arange(0, 100)
...
def generate_vsm_data() -> pd.DataFrame:
    return pd.DataFrame([
        {"Step": "Accessioning", "CycleTime": 10, "WaitTime": 120, "ValueAdded": True}, {"Step": "Extraction", "CycleTime": 90, "WaitTime": 30, "ValueAdded": True},
        {"Step": "Lib Prep", "CycleTime": 240, "WaitTime": 1200, "ValueAdded": True}, {"Step": "Sequencing", "CycleTime": 1440, "WaitTime": 2880, "ValueAdded": True},
        {"Step": "Biofx", "CycleTime": 180, "WaitTime": 60, "ValueAdded": True}, {"Step": "Reporting", "CycleTime": 30, "WaitTime": 240, "ValueAdded": True}])
...
# ==============================================================================
# SECTION 3: VISUALIZATION HELPERS (MAJOR UPGRADES)
# ==============================================================================
...
def plot_ctq_tree_visual() -> graphviz.Digraph:
    """FIX: Graphviz rendering corrected for high-contrast readability."""
    ...
    dot.node('Need', 'Clinician Need:\n"Reliable Early CRC Detection"', fillcolor=hex_to_rgba(COLORS['accent'], 0.1), color=COLORS['accent'], penwidth='2')
    dot.node('Driver1', 'High Sensitivity', fillcolor=hex_to_rgba(COLORS['primary'], 0.1), color=COLORS['primary'], penwidth='2')
    dot.node('Driver2', 'High Specificity', fillcolor=hex_to_rgba(COLORS['primary'], 0.1), color=COLORS['primary'], penwidth='2')
    dot.node('Driver3', 'Fast Turnaround', fillcolor=hex_to_rgba(COLORS['primary'], 0.1), color=COLORS['primary'], penwidth='2')
    dot.edge('Need', 'Driver1'); dot.edge('Need', 'Driver2'); dot.edge('Need', 'Driver3')
    dot.node('CTQ1', 'CTQ:\nLOD < 0.1% VAF', fillcolor=hex_to_rgba(COLORS['secondary'], 0.1), color=COLORS['secondary'], penwidth='2')
    dot.node('CTQ2', 'CTQ:\nAnalytical Specificity > 99.5%', fillcolor=hex_to_rgba(COLORS['secondary'], 0.1), color=COLORS['secondary'], penwidth='2')
    dot.node('CTQ3', 'CTQ:\nSample-to-Report < 5 days', fillcolor=hex_to_rgba(COLORS['secondary'], 0.1), color=COLORS['secondary'], penwidth='2')
...
def plot_causal_discovery_visual() -> graphviz.Digraph:
    ...
    dot.node('ReagentLot', 'Reagent<br>Lot', fillcolor=hex_to_rgba(COLORS['primary'], 0.1), color=COLORS['primary'], penwidth='2')
    dot.node('DNAnq', 'DNA<br>Input (ng)', fillcolor=hex_to_rgba(COLORS['primary'], 0.1), color=COLORS['primary'], penwidth='2')
    dot.node('LigationTime', 'Ligation<br>Time', fillcolor=hex_to_rgba(COLORS['secondary'], 0.1), color=COLORS['secondary'], penwidth='2')
    dot.node('AdapterDimer', 'Adapter-Dimer<br>Formation (%)', fillcolor=hex_to_rgba(COLORS['accent'], 0.15), color=COLORS['accent'], penwidth='2')
...
def plot_voc_bubble_chart() -> go.Figure:
    """UPGRADE: Replaced Treemap with a Bubble Chart for a more scientific landscape view."""
    ...
    fig = px.scatter(
        df, x='Topic', y='Sentiment', size='Count', color='Category',
    ...
    fig.update_layout(
        ...
        plot_bgcolor='white',
        yaxis=dict(range=[-1, 1], gridcolor=COLORS['light_gray']),
...
def plot_gage_rr_pareto() -> go.Figure:
    """UPGRADE: Replaced stacked bar with a Pareto chart for superior technical insight."""
    data = {'Source of Variation': ['Assay Variation (Biology)', 'Repeatability (Sequencer)', 'Reproducibility (Operator)'],
            'Contribution (%)': [92, 5, 3]}
    ...
    fig.add_trace(
        go.Bar(x=df['Source of Variation'], y=df['Contribution (%)'], name='Contribution',
               marker_color=[COLORS['primary'], COLORS['warning'], COLORS['accent']]),
        secondary_y=False
    )
    # Line chart for cumulative percentage
    fig.add_trace(
        go.Scatter(x=df['Source of Variation'], y=df['Cumulative Percentage'], name='Cumulative %',
                   mode='lines+markers', line_color=COLORS['dark_gray']),
        secondary_y=True
    )
    fig.update_layout(
...
def plot_process_mining_graph() -> graphviz.Digraph:
    ...
    dot.node('start', 'Sample\nReceived', shape='circle', style='filled', fillcolor=hex_to_rgba(COLORS['success'], 0.1), color=COLORS['success'], penwidth='2')
    dot.node('end', 'Report\nSent', shape='doublecircle', style='filled', fillcolor=hex_to_rgba(COLORS['dark_gray'], 0.1), color=COLORS['dark_gray'], penwidth='2')
    node_fill = hex_to_rgba(COLORS['primary'], 0.1)
    node_color = COLORS['primary']
    dot.node('A', 'DNA Extraction', fillcolor=node_fill, color=node_color, penwidth='2')
    dot.node('B', 'Library Prep', fillcolor=node_fill, color=node_color, penwidth='2')
    dot.node('C', 'Sequencing', fillcolor=node_fill, color=node_color, penwidth='2')
    dot.node('D', 'Bioinformatics', fillcolor=node_fill, color=node_color, penwidth='2')
    dot.node('E', 'QC Fail:\nRe-Prep', style='rounded,filled', fillcolor=hex_to_rgba(COLORS['danger'], 0.15), color=COLORS['danger'], penwidth='2')
...
def plot_capability_analysis_pro(data: np.ndarray, lsl: float, usl: float) -> Tuple[go.Figure, float, float]:
    """Calculates capability and returns the main histogram. Visual indicators are now handled in the page."""
...
def plot_fishbone_diagram() -> graphviz.Digraph:
    ...
    dot.node('Effect', 'Low Library Yield', shape='box', style='filled', fillcolor=hex_to_rgba(COLORS['danger'], 0.15), fontcolor=COLORS['text'], color=COLORS['danger'], penwidth='2')
...
# All other plotting functions remain as they were, since they were already robust.
# ... (plot_pareto_chart, plot_anova_groups, etc. are omitted for brevity but are unchanged) ...
...
def plot_permutation_test(df: pd.DataFrame, n_permutations: int = 1000) -> go.Figure:
    ...
    for _ in range(n_permutations):
        np.random.shuffle(concat_data); perm_diffs.append(concat_data[:len(g1_data)].mean() - concat_data[len(g1_data):].mean())
...
def train_and_plot_regression_models(df: pd.DataFrame) -> Tuple[go.Figure, RandomForestRegressor, pd.DataFrame]:
    ...
    lin_reg = LinearRegression().fit(X, y); y_pred_lin = lin_reg.predict(X); r2_lin = lin_reg.score(X, y)
...
def plot_rul_prediction(df: pd.DataFrame) -> go.Figure:
    ...
    if len(failure_indices) > 0:
        ttf = future_time[failure_indices[0]]; rul_text = f"Predicted RUL: {ttf - current_time:.0f} Runs"
    else:
        ttf = 120; rul_text = f"RUL: >{120-current_time} Runs"
    fig = go.Figure()
...
def plot_shewhart_chart(df: pd.DataFrame) -> go.Figure:
    ...
    ucl, lcl = mean + 3 * std_dev, mean - 3 * std_dev; violations = df[(df['Yield_ng'] > ucl) | (df['Yield_ng'] < lcl)]
    fig = go.Figure()
...
def plot_ewma_chart(df: pd.DataFrame, lambda_val: float = 0.2) -> go.Figure:
    mean, std_dev = df['Yield_ng'].iloc[:75].mean(), df['Yield_ng'].iloc[:75].std(ddof=1)
    df['ewma'] = df['Yield_ng'].ewm(span=(2 / lambda_val) - 1).mean(); n = np.arange(1, len(df) + 1)
    ...
    violations = df[(df['ewma'] > ucl_ewma) | (df['ewma'] < lcl_ewma)]; fig = go.Figure()
...
def plot_cusum_chart(df: pd.DataFrame, k: float = 0.5, h: float = 5.0) -> go.Figure:
    ...
    target, k_val, h_val = mean, k * std, h * std; sh, sl = np.zeros(len(df)), np.zeros(len(df))
    for i in range(1, len(df)):
        sh[i] = max(0, sh[i - 1] + df['Yield_ng'].iloc[i] - target - k_val)
        sl[i] = min(0, sl[i - 1] + df['Yield_ng'].iloc[i] - target + k_val)
    violations = np.where((sh > h_val) | (sl < -h_val))[0]; fig = go.Figure()
...
def plot_hotelling_t2_chart() -> go.Figure:
    df = generate_hotelling_data(); X_in_control = df.iloc[:80, :]
    mean_vec = X_in_control.mean().values; inv_cov_mat = np.linalg.inv(np.cov(X_in_control.T))
    ...
    ucl = (p * (n + 1) * (n - 1)) / (n * (n - p)) * f_dist.ppf(1 - alpha, p, n - p); fig = go.Figure()
...
def render_workflow_step(phase_name: str, phase_class: str, classical_tools: List[str], ml_tools: List[str]) -> str:
    ...
    <div class="workflow-step {phase_class}">
        ...
        <div style="display:flex;justify-content:space-between;">
            <div style="flex:1;margin-right:10px;" class="tool-col tool-col-classical"><h5>Classical Tools (Rigor & Validation)</h5><ul>{classical_list}</ul></div>
            <div style="flex:1;margin-left:10px;" class="tool-col tool-col-ml"><h5>ML/Bio-AI Augmentation (Scale & Discovery)</h5><ul>{ml_list}</ul></div>
        </div>
    </div>
    """
