...
def show_define_phase():
    ...
    with st.container(border=True):
        ...
        with tab1:
            st.markdown("##### **Classical Tool: CTQ Tree**"); st.info("A decomposition tool to break down a broad clinical need (e.g., reliable liquid biopsy) into specific, measurable performance metrics like **Analytical Sensitivity (LOD), Specificity, and Precision (CV%)**."); st.graphviz_chart(plot_ctq_tree_visual())
        with tab2:
            st.markdown("##### **Classical Tool: Kano Model**"); st.info("A framework for prioritizing diagnostic features. **Basic** needs might be detecting a known cancer mutation. **Performance** could be accurate VAF quantification. An **Excitement** feature could be discovering a novel, actionable co-mutation."); st.plotly_chart(plot_kano_visual(), use_container_width=True)
        with tab3:
            st.markdown("##### **ML Augmentation: NLP for Scientific Literature**"); st.info("Using algorithms (e.g., SciBERT) to analyze thousands of publications to automatically extract prevalent biomarkers, competing methodologies, and reported performance benchmarks, massively accelerating the literature review process."); st.plotly_chart(plot_voc_treemap(), use_container_width=True)
...
def show_measure_phase():
    ...
    with st.container(border=True):
        ...
        with col1: st.markdown("##### **Classical Tool: Gage R&R**"); st.info("A designed experiment to partition measurement variance into **Repeatability** (e.g., one sequencer's variation) and **Reproducibility** (e.g., variation between lab technicians).")
        with col2: st.plotly_chart(plot_gage_rr_variance_components(), use_container_width=True)
    with st.container(border=True):
        ...
        with tab1: st.markdown("##### **Classical Tool: Value Stream Mapping (VSM)**"); st.info("A manual, observational flowchart of the lab process, capturing hands-on and wait times to identify physical bottlenecks and streamline the workflow."); st.plotly_chart(plot_vsm(), use_container_width=True)
        with tab2: st.markdown("##### **ML Augmentation: Process Mining on LIMS Data**"); st.info("Algorithms that automatically discover the real lab workflow by analyzing event logs from a **Laboratory Information Management System (LIMS)**. It discovers all re-tests, QC failures, and true bottlenecks hidden in manual maps."); st.graphviz_chart(plot_process_mining_graph())
    with st.container(border=True):
        ...
        st.sidebar.header("Assay Capability Simulator"); st.sidebar.markdown("Adjust assay performance and spec limits to see the impact on capability."); lsl = st.sidebar.slider("Lower Spec Limit (LSL)", 0.5, 2.0, 0.8, key="m_lsl", help="e.g., Minimum required signal-to-noise"); usl = st.sidebar.slider("Upper Spec Limit (USL)", 8.0, 10.0, 9.0, key="m_usl", help="e.g., Maximum tolerable background"); process_mean = st.sidebar.slider("Assay Mean (μ)", 2.0, 8.0, 4.0, key="m_mean"); process_std = st.sidebar.slider("Assay Std Dev (σ)", 0.2, 2.0, 0.5, key="m_std")
        ...
        fig_cp_gauge, fig_cpk_gauge = plot_capability_metrics(cp, cpk) # NEW function call
...
# ... The rest of the pages (Analyze, Improve, Control, etc.) follow the same correct and synchronized structure ...
def show_analyze_phase():
    ...
    with st.container(border=True):
        ...
        col1, col2 = st.columns(2)
        with col1: st.markdown("##### **Classical Tool: Fishbone Diagram**"); st.info("A structured brainstorming tool to organize potential causes of an assay problem (e.g., 'Low Library Yield') into categories like 'Reagents', 'Equipment', 'Method', etc."); st.graphviz_chart(plot_fishbone_diagram())
        with col2: st.markdown("##### **Classical Tool: Pareto Chart**"); st.info("A chart to identify the 'vital few' failure modes (e.g., 'Adapter-dimer formation', 'Low PCR efficiency'). This focuses effort on the highest-frequency problems."); st.plotly_chart(plot_pareto_chart(), use_container_width=True)
    with st.container(border=True):
        ...
        st.sidebar.header("Group Comparison Simulator"); st.sidebar.markdown("Adjust reagent lot means to see if the difference in library yield becomes statistically significant."); mean1 = st.sidebar.slider("Lot A Mean Yield (ng/µL)",18.0,22.0,19.5,0.1,key='a1'); mean2 = st.sidebar.slider("Lot B Mean Yield (ng/µL)",18.0,22.0,20.0,0.1,key='a2'); mean3 = st.sidebar.slider("Lot C Mean Yield (ng/µL)",18.0,22.0,20.5,0.1,key='a3'); anova_data = generate_anova_data(means=[mean1,mean2,mean3],stds=[0.8,0.8,0.8],n=20)
        tab1, tab2 = st.tabs(["🔬 Classical: ANOVA", "💻 ML Augmentation: Permutation Testing"])
        with tab1:
            st.markdown("##### **Classical: Analysis of Variance (ANOVA)**"); st.info("A test to determine if significant differences exist between the mean yields of different reagent lots. Assumes normality and equal variances."); fig_anova, p_val = plot_anova_groups(anova_data); st.plotly_chart(fig_anova, use_container_width=True)
        ...
        with tab2: st.markdown("##### **ML Augmentation: Permutation Testing**"); st.info("A non-parametric, computational method that makes no assumptions about the data's distribution. More robust for the often non-normal, small-sample data common in biotech R&D."); st.plotly_chart(plot_permutation_test(anova_data), use_container_width=True)
    with st.container(border=True):
        ...
        df_reg = generate_nonlinear_data(); col3, col4 = st.columns(2)
        with col3: st.markdown("##### **Classical: Multiple Linear Regression**"); st.info("Models the linear relationship between parameters and output. Simple, but often fails to capture the complex, non-linear biology of an assay."); fig_reg, _, _ = plot_regression_comparison_pro(df_reg); st.plotly_chart(fig_reg, use_container_width=True)
        with col4: st.markdown("##### **ML Augmentation: Ensemble Models & XAI (SHAP)**"); st.info("Ensemble models (**Random Forest, Gradient Boosting**) capture complex, non-linear relationships. **SHAP** is then used to understand the model's logic and rank parameter importance."); _, model, X_reg = plot_regression_comparison_pro(df_reg); fig_shap = plot_shap_summary(model, X_reg); st.plotly_chart(fig_shap, use_container_width=True)
...
def show_improve_phase():
    ...
    with st.container(border=True):
        ...
        with tab1:
            st.markdown("##### **Classical: Design of Experiments (DOE)**"); st.info("A structured method for efficiently changing multiple parameters simultaneously to determine their main and interaction effects. The gold standard for physical lab experimentation, but impractical for high-dimensional problems."); doe_data = generate_doe_data(); fig_doe_main, fig_doe_interaction = plot_doe_effects(doe_data); col1, col2 = st.columns(2)
        ...
        with tab2:
            st.markdown("##### **ML Augmentation: Bayesian Optimization**"); st.info("An intelligent search algorithm for finding the global optimum of an expensive-to-evaluate function (e.g., a full NGS run). It builds a model of the assay's performance and uses it to intelligently select the most informative next experiment to run."); st.sidebar.header("Bayesian Opt. Simulator"); st.sidebar.markdown("Let the algorithm intelligently choose the next experiment to run to find the maximum on-target rate.")
            @st.cache_data 
    ...
    with st.container(border=True):
        ...
        with col3: st.markdown("##### **Classical: FMEA**"); st.info("Failure Mode and Effects Analysis is a systematic, team-based risk assessment of the assay protocol. It brainstorms failure modes (e.g., 'Reagent Contamination') and ranks them by a **Risk Priority Number (RPN)**."); st.plotly_chart(plot_fmea_table(), use_container_width=True)
        with col4: st.markdown("##### **ML Augmentation: Predictive Instrument Maintenance**"); st.info("Using ML models trained on sensor data from lab equipment (e.g., sequencer laser power) to predict degradation and estimate **Remaining Useful Life (RUL)** before a failure compromises an entire batch."); st.plotly_chart(plot_rul_prediction(generate_sensor_degradation_data()), use_container_width=True)
...
def show_control_phase():
    ...
    st.markdown("---")
    with st.container(border=True):
        st.subheader("1. Monitoring for Stability: Statistical Process Control (SPC) for QC"); st.markdown("Control charts monitor positive/negative controls over time, distinguishing 'common cause' variation from 'special cause' variation that signals a problem."); st.sidebar.header("QC Simulator"); st.sidebar.markdown("Introduce a shift in a positive control standard and see which chart detects it faster."); shift_mag=st.sidebar.slider("Magnitude of Shift (in Std Devs)",0.2,3.0,0.8,0.1,key="ctrl_shift_mag"); ewma_lambda=st.sidebar.slider("EWMA Lambda (λ)",0.1,0.5,0.2,0.05,help="Higher λ reacts faster to shifts."); chart_data=generate_control_chart_data(shift_point=75,shift_magnitude=shift_mag)
        tab1,tab2,tab3=st.tabs(["📊 Classical: Levey-Jennings Chart","📈 Advanced Classical: EWMA/CUSUM","🤖 ML: Multivariate QC"])
        with tab1: st.markdown("##### **Classical: Levey-Jennings Chart (Shewhart)**"); st.info("The standard QC chart in clinical labs. It plots QC measurements over time with control limits at ±2σ and ±3σ. Excellent for detecting large, sudden shifts."); st.plotly_chart(plot_shewhart_chart(chart_data),use_container_width=True)
        with tab2: st.markdown("##### **Advanced Classical: EWMA & CUSUM Charts**"); st.info("These charts have 'memory,' making them highly effective at detecting small, sustained drifts (e.g., slow reagent degradation) that Levey-Jennings charts would miss."); st.plotly_chart(plot_ewma_chart(chart_data,lambda_val=ewma_lambda),use_container_width=True); st.plotly_chart(plot_cusum_chart(chart_data),use_container_width=True)
        with tab3: st.markdown("##### **ML Augmentation: Multivariate QC**"); st.info("An NGS assay has many correlated QC metrics (e.g., `% Mapped Reads`, `% Duplication`). ML can monitor the 'health' of the entire QC profile at once, flagging abnormalities even if each individual metric is within its own limits."); st.plotly_chart(plot_hotelling_t2_chart(),use_container_width=True)
    with st.container(border=True): st.subheader("2. Formalizing the Gains: The Control Plan & SOPs"); st.info("The Control Plan is a living document that details the QC methods, responsibilities, and reaction plan for any out-of-control signal. It is codified in the lab's Standard Operating Procedures (SOPs)."); st.plotly_chart(plot_control_plan(),use_container_width=True)
...
def show_comparison_matrix():
    ...
    st.markdown("---")
    with st.container(border=True): st.subheader("Strengths Profile: A Multi-Dimensional View"); st.markdown("This radar chart compares the two methodologies across key attributes relevant to assay development. The different 'shapes' of their capabilities highlight their complementary nature."); st.plotly_chart(plot_comparison_radar(),use_container_width=True)
    with st.container(border=True): st.subheader("The Verdict: Which Approach Excels for Which Task?"); st.markdown("This chart provides a clear, decisive verdict for common use cases in a biotech R&D setting."); st.plotly_chart(plot_verdict_barchart(),use_container_width=True)
...
def show_hybrid_strategy():
    ...
    st.markdown("---")
    with st.container(border=True): st.subheader("The Philosophy of Synergy: Inference + Prediction"); st.markdown("Neither methodology is a silver bullet. True power lies in their integration. Classical statistics provides the **rigor for inference and causality**, while machine learning provides the **power for prediction and scale**."); st.plotly_chart(plot_synergy_diagram(),use_container_width=True)
    with st.container(border=True):
        st.subheader("Interactive Solution Recommender"); st.info("💡 Select a common R&D scenario to see the recommended hybrid approach and expert rationale."); guidance_data=get_guidance_data(); scenarios=list(guidance_data.keys()); selected_scenario=st.selectbox("Choose your scenario:",scenarios,label_visibility="collapsed")
        if selected_scenario: st.markdown(f"##### Recommended Approach: {guidance_data[selected_scenario]['approach']}"); st.markdown(f"**Rationale:** {guidance_data[selected_scenario]['rationale']}")
    with st.container(border=True):
        st.subheader("A Unified, Modern R&D Workflow"); st.markdown("This workflow demonstrates how to embed ML augmentation at each step of the traditional assay development cycle."); st.markdown(get_workflow_css(),unsafe_allow_html=True); st.markdown('<div class="workflow-container">',unsafe_allow_html=True)
        st.markdown(render_workflow_step(phase_name="🌀 1. Define",phase_class="step-define",classical_tools=["Assay Design Plan","SIPOC of Workflow","Kano Model","CTQ Tree (TPP)"],ml_tools=["NLP for Literature Review","Causal Discovery from Pilot Data","Patient Stratification"]),unsafe_allow_html=True); st.markdown('<div class="workflow-arrow">⬇️</div>',unsafe_allow_html=True)
        st.markdown(render_workflow_step(phase_name="🔬 2. Measure",phase_class="step-measure",classical_tools=["Gage R&R (MSA)","Process Capability (Cpk)","VSM of Lab Process"],ml_tools=["Process Mining on LIMS","Kernel Density Estimation (KDE)","Assay Drift Detection"]),unsafe_allow_html=True); st.markdown('<div class="workflow-arrow">⬇️</div>',unsafe_allow_html=True)
        st.markdown(render_workflow_step(phase_name="📈 3. Analyze",phase_class="step-analyze",classical_tools=["Hypothesis Testing (ANOVA)","Pareto Analysis","Fishbone Diagram","Linear Regression"],ml_tools=["Biomarker Feature Importance (SHAP)","Ensemble Models","Permutation Testing"]),unsafe_allow_html=True); st.markdown('<div class="workflow-arrow">⬇️</div>',unsafe_allow_html=True)
        st.markdown(render_workflow_step(phase_name="⚙️ 4. Improve",phase_class="step-improve",classical_tools=["Design of Experiments (DOE)","FMEA","Pilot Validation"],ml_tools=["Bayesian Optimization","Predictive Maintenance (RUL)","In Silico Surrogate Models"]),unsafe_allow_html=True); st.markdown('<div class="workflow-arrow">⬇️</div>',unsafe_allow_html=True)
        st.markdown(render_workflow_step(phase_name="📡 5. Control",phase_class="step-control",classical_tools=["Levey-Jennings Charts (SPC)","Control Plan & SOPs"],ml_tools=["Multivariate QC (Hotelling's T²)","Real-time Anomaly Detection","Automated Batch Release"]),unsafe_allow_html=True); st.markdown('</div>',unsafe_allow_html=True)
