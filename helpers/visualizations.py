...
def plot_project_charter_visual() -> go.Figure:
    ...
    fig = _create_network_fig(height=350, x_range=[0, 1], y_range=[0, 1])
    ...
    statements = {"Problem Statement": (0.05, 0.45, "Colorectal Cancer (CRC) requires earlier detection methods.<br>Current methods are invasive or lack sensitivity for early-stage disease.", "left"), "Goal Statement": (0.95, 0.45, "Develop and validate a cfDNA-based NGS assay for early-stage<br>CRC detection with >90% sensitivity at 99.5% specificity.", "right")}
    for title, (x, y, txt, anchor) in statements.items():
        ...
        fig.add_annotation(x=x, y=y - 0.1, text=txt, showarrow=False, align=anchor, xanchor=anchor, yanchor='top')
    return fig
...
def plot_sipoc_visual() -> go.Figure:
    ...
    fig.update_layout(title_text="<b>SIPOC Diagram:</b> High-Level NGS Assay Workflow", margin=dict(l=10, r=10, t=80, b=10))
    return fig
...
def plot_voc_bubble_chart(df_voc: pd.DataFrame) -> go.Figure:
    ...
    fig = px.scatter(
        df_voc, x='Topic', y='Sentiment', size='Count', color='Category', hover_name='Topic', size_max=60,
    ...
    fig.update_layout(title="<b>NLP Landscape:</b> Scientific Literature Analysis", yaxis=dict(range=[-1, 1]), xaxis=dict(showgrid=False), legend_title_text='Topic Category')
...
def plot_dfmea_table_pro(df: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure(data=[go.Table(
        columnwidth=[3, 2.5, 0.8, 2.5, 0.8, 2, 0.8, 0.8],
        header=dict(values=[f'<b>{col.replace(" ", "<br>")}</b>' for col in df_display.columns], fill_color=COLORS['dark_gray'], font=dict(color='white', size=12), align='center', height=50, line_color='darkslategray'),
        cells=dict(values=[df_display[k] for k in df_display], fill_color=[['white']*len(df)]*(len(df.columns)-1) + [rpn_colors], align='left', font_size=11, height=40, line_color='darkslategray')
    )])
    fig.update_layout(title="<b>Design FMEA:</b> Proactive Risk Analysis", margin=dict(l=10, r=10, t=80, b=10))
    return fig
...
def plot_capability_analysis_pro(data: np.ndarray, lsl: float, usl: float) -> Tuple[go.Figure, float, float]:
    ...
    if data is None or len(data) < 2:
        return go.Figure().update_layout(title_text="<b>Error:</b> Insufficient data for capability analysis."), 0, 0
...
def plot_fishbone_plotly() -> go.Figure:
    ...
    for name, data in bones.items():
        ...
        for i, cause in enumerate(data['causes']):
            sub_x_start = x_start + (1.5 + i*1.0) * np.cos(angle_rad)
            sub_y_start = y_start + (1.5 + i*1.0) * np.sin(angle_rad)
            ...
            fig.add_annotation(x=sub_x_start, y=sub_y_start, text=cause, showarrow=False, font=dict(size=10), textangle=0, bgcolor='white', xshift=10 if data['angle'] > 0 else -10)
...
def plot_pareto_chart(df_pareto: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(title_text="<b>Pareto Chart:</b> Identifying the 'Vital Few' Failure Modes", xaxis_title="QC Failure Mode", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
...
def plot_nlp_on_capa_logs(df_topics: pd.DataFrame) -> go.Figure:
    ...
    fig = px.bar(
        df_topics, x='Frequency', y='Topic', orientation='h', color='Topic',
        labels={'y': 'Identified Failure Theme', 'x': 'Frequency'}, title="<b>NLP Topic Modeling on CAPA & Deviation Logs</b>",
...
def plot_regression_comparison(model_results: dict) -> go.Figure:
    ...
    X, y = model_results['X'], model_results['y']
...
def plot_cusum_chart(df_control: pd.DataFrame, k: float = 0.5, h: float = 5.0) -> go.Figure:
    ...
    fig.update_layout(title=f'<b>CUSUM Chart:</b> Accumulating Process Shift Information', xaxis_title='Batch ID', yaxis_title='Cumulative Sum')
    return fig
...
def plot_hotelling_t2_chart(df_hotelling: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(title="<b>Hotelling's T² Chart:</b> Multivariate Process Control", xaxis_title="Sample Number", yaxis_title="T² Statistic")
    return fig
...
def plot_control_plan_pro() -> go.Figure:
    ...
    fig = go.Figure(data=[go.Table(
        header=dict(values=[f"<b>{c}</b>" for c in df.columns], fill_color=COLORS['dark_gray'], font=dict(color='white', size=12), align='left', height=40, line_color='darkslategray'),
        cells=dict(values=[df[c] for c in df.columns], align='left', font_size=11, height=30, line_color='darkslategray', fill_color=[['#f9f9f9', 'white'] * 4])
    )])
    fig.update_layout(title="<b>Assay Control Plan:</b> Formalizing QC Procedures", margin=dict(l=10, r=10, t=80, b=10))
    return fig
...
def plot_adverse_event_clusters(df_clustered: pd.DataFrame) -> go.Figure:
    ...
    fig = px.scatter(
        df_clustered, x='x_pca', y='y_pca', color='cluster_name', hover_data=['description'],
        title="<b>ML Clustering of Adverse Event Narratives for Signal Detection</b>", labels={'color': 'Event Cluster'}
    )
    fig.update_layout(xaxis_title="PCA Component 1", yaxis_title="PCA Component 2", xaxis=dict(showticklabels=False, zeroline=False), yaxis=dict(showticklabels=False, zeroline=False))
    return fig
...
def plot_pccp_monitoring(df_pccp: pd.DataFrame) -> go.Figure:
    ...
    fig.add_annotation(x=85, y=0.87, text="<b>Retraining & Revalidation<br>Triggered per PCCP</b>", showarrow=True, arrowhead=1, ax=0, ay=-40, bgcolor="rgba(255,255,255,0.8)")
    fig.update_layout(title="<b>PCCP Monitoring for an AI/ML Device (SaMD)</b>", xaxis_title="Days Since Deployment", yaxis_title="Model Area Under Curve (AUC)", legend=dict(x=0.01, y=0.01, yanchor='bottom'))
    return fig
...
def plot_comparison_radar() -> go.Figure:
    ...
    fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 5.5])), title="<b>Strengths Profile:</b> Classical vs. ML for Biotech R&D", legend=dict(orientation="h", yanchor="bottom", y=-0.2, xanchor="center", x=0.5))
    return fig
...
def plot_verdict_barchart() -> go.Figure:
    ...
    fig.update_layout(xaxis=dict(tickvals=[-1, 1], ticktext=['<b>Winner: Classical Stats</b>', '<b>Winner: Machine Learning</b>'], tickfont=dict(size=14), range=[-1.5, 1.5]), yaxis_title=None, bargap=0.4, showlegend=False)
    return fig
...
def plot_synergy_diagram() -> go.Figure:
    ...
    fig.update_layout(title="<b>The Hybrid Philosophy:</b> Combining Strengths", margin=dict(t=50, b=10, l=10, r=10))
    return fig
