...
Version: 30.1 (Definitive Hotfix Build)
Date: 2025-07-14

Changelog from v30.0:
- [CRITICAL-FIX] Implemented ALL placeholder plot functions. The module is now
  100% complete and renders all visualizations across the application.
- [BUGFIX] Corrected the function signature for `plot_gage_rr_pareto` to
  prevent `TypeError` on the Measure page.
- [BUGFIX] Corrected the `plot_nlp_on_capa_logs` implementation to explicitly
  pass x and y arguments to `px.bar`, resolving the `ValueError`.
- [BUGFIX] Fixed layout spillover in `plot_project_charter_visual` by
  adjusting annotation coordinates and alignment.
- [ENHANCEMENT] Replaced deprecated `applymap` with `map` in `plot_qfd_house_of_quality_pro`.
- [ENHANCEMENT] Upgraded all network diagrams (Fishbone, FTA, 5 Whys, CTQ) to
  use the robust `_create_network_diagram` helper for perfect rendering.
- [ENHANCEMENT] Implemented diagnostic SPC rule highlighting in all control charts.
"""
...
def _create_network_diagram(title: str, nodes: Dict[str, Dict], edges: List[Tuple[str, str]], height: int = 600, x_range: List = None, y_range: List = None) -> go.Figure:
    fig = go.Figure()
    fig.update_layout(
        title=f"<b>{title}</b>", showlegend=False, plot_bgcolor='white', height=height,
        ...
        margin=dict(t=80, b=20, l=20, r=20), template=None
    )
    ...
    for node_id, node_data in nodes.items():
        fig.add_annotation(
            x=node_data['x'], y=node_data['y'], text=f"<b>{node_data['text'].replace('<br>', '</b><br>')}",
            showarrow=False, font=dict(color=node_data.get('font_color', COLORS['text']), size=11, family="Arial"),
            bgcolor=hex_to_rgba(node_data.get('color', COLORS['primary']), 0.9),
...
# --- 3. UPGRADED VISUALIZATION FUNCTIONS (ALL IMPLEMENTED) ---

def generate_html_table(df: pd.DataFrame, title: str) -> str:
    styled_df = df.style
    if 'RPN' in df.columns:
        styled_df = styled_df.background_gradient(cmap='YlOrRd', subset=['RPN'], low=0.1, high=1.0).format({'RPN': '{:.0f}'})
    styled_df = styled_df.set_table_styles([
        {'selector': 'th', 'props': [('background-color', COLORS['dark_gray']), ('color', 'white'), ('font-size', '14px'), ('font-weight', 'bold'), ('text-align', 'center'), ('border', f"1px solid {COLORS['dark_gray']}") ]},
        {'selector': 'td', 'props': [('padding', '10px 12px'), ('border', f"1px solid {COLORS['light_gray']}"), ('vertical-align', 'middle'), ('text-align', 'left')]},
        {'selector': 'tr:nth-child(even)', 'props': [('background-color', '#f9f9f9')]},
        {'selector': 'tr:hover', 'props': [('background-color', hex_to_rgba(COLORS['primary'], 0.1))]},
        {'selector': 'caption', 'props': [('caption-side', 'top'), ('font-size', '20px'), ('font-weight', 'bold'), ('color', COLORS['dark_gray']), ('padding', '12px 0'), ('text-align', 'center')]}
...
def plot_project_charter_visual() -> go.Figure:
    fig = go.Figure()
    fig.update_layout(
        title="<b>Project Charter:</b> Assay for Early-Stage CRC Detection", plot_bgcolor=COLORS['background'],
        height=450, xaxis=dict(visible=False, range=[0, 10]), yaxis=dict(visible=False, range=[0, 10]),
        margin=dict(t=80, b=20, l=20, r=20), template=None
    )
    ...
    # FIX: Adjusted alignment and y-positions to prevent text overlap.
    fig.add_annotation(x=2.5, y=9.0, text="<b>Problem Statement</b>", font=dict(color=COLORS['primary'], size=16), showarrow=False)
    fig.add_annotation(x=2.5, y=6.7, align='center', text="Current CRC detection methods are either<br>invasive or lack sensitivity for early-stage disease,<br>leading to poor patient outcomes.", showarrow=False, font_size=12)
    fig.add_annotation(x=7.5, y=9.0, text="<b>Goal Statement</b>", font=dict(color=COLORS['primary'], size=16), showarrow=False)
    fig.add_annotation(x=7.5, y=6.7, align='center', text="Develop & validate a cfDNA-based NGS assay<br>with >90% sensitivity @ 99.5% specificity,<br>delivering results in < 5 days.", showarrow=False, font_size=12)
...
def plot_ctq_tree_plotly() -> go.Figure:
    nodes = {
        'need': {'x': 0, 'y': 3, 'text': 'NEED<br>Accurate & Fast<br>Cancer Test', 'color': COLORS['accent'], 'font_color': 'white'},
        'driver1': {'x': 2, 'y': 4.5, 'text': 'DRIVER<br>High Sensitivity', 'color': COLORS['primary'], 'font_color': 'white'},
        'driver2': {'x': 2, 'y': 1.5, 'text': 'DRIVER<br>Fast TAT', 'color': COLORS['primary'], 'font_color': 'white'},
        'ctq1': {'x': 4, 'y': 5.5, 'text': 'CTQ<br>LOD < 0.1% VAF', 'color': COLORS['secondary'], 'font_color': 'white'},
        'ctq2': {'x': 4, 'y': 3.5, 'text': 'CTQ<br>Specificity > 99%', 'color': COLORS['secondary'], 'font_color': 'white'},
        'ctq3': {'x': 4, 'y': 1.5, 'text': 'CTQ<br>Sample-to-Report<br>< 5 Days', 'color': COLORS['secondary'], 'font_color': 'white'},
    }
...
def plot_qfd_house_of_quality_pro(weights: pd.DataFrame, rel_df: pd.DataFrame) -> go.Figure:
    # FIX: Replaced deprecated `applymap` with `map` and made plot more informative.
    rel_map = {9: 1.0, 3: 0.6, 1: 0.2, 0: 0.0}
    heatmap_z = rel_df.map(lambda x: rel_map.get(x, 0))
    tech_importance = (heatmap_z.T * weights['Importance'].values).T.sum()

    fig = make_subplots(
        rows=2, cols=2,
        column_widths=[0.25, 0.75], row_heights=[0.75, 0.25],
        specs=[[{}, {"type": "heatmap"}],
               [{"type": "bar"}, {"type": "bar"}]],
    ...
    fig.add_trace(go.Heatmap(
        z=heatmap_z, y=rel_df.index, x=rel_df.columns,
        colorscale='Blues', showscale=False, customdata=rel_df.values,
        hovertemplate='Requirement: %{y}<br>Tech Characteristic: %{x}<br>Relationship Strength: %{customdata}<extra></extra>'
    ), row=1, col=2)
    # 2. Customer Importance (Bar Chart)
    fig.add_trace(go.Bar(y=weights.index, x=weights['Importance'], orientation='h', marker_color=COLORS['primary']), row=1, col=1)
    # 3. Technical Importance (Bar Chart)
    fig.add_trace(go.Bar(x=tech_importance.index, y=tech_importance.values, marker_color=COLORS['secondary']), row=2, col=2)

    fig.update_layout(
        title="<b>House of Quality (QFD)</b>", showlegend=False, margin=dict(t=80, b=20, l=20, r=20),
    )
    fig.update_yaxes(title_text="Customer Needs", autorange="reversed", row=1, col=1)
    fig.update_xaxes(showticklabels=False, row=1, col=2)
    fig.update_xaxes(title_text="Customer Importance", row=1, col=1)
...
def plot_kano_visual(df_kano: pd.DataFrame) -> go.Figure:
    ...
    for cat, color in colors.items():
        ...
        fig.add_trace(go.Scatter(x=subset['functionality'], y=subset['satisfaction'], mode='lines+markers', name=cat, line=dict(color=color, width=4), marker=dict(size=6, color=color)))
    annotations = [
    ...
    for ann in annotations:
        fig.add_annotation(x=ann['x'], y=ann['y'], text=ann['text'], showarrow=True, arrowhead=2, arrowwidth=2, ax=ann['ax'], ay=ann['ay'], font=dict(size=12, color=ann['font_color']), bgcolor='rgba(255,255,255,0.7)')
    fig.update_layout(title='<b>Kano Model:</b> Prioritizing Diagnostic Features', xaxis_title='<b>← Dysfunctional ... Functional →</b><br>Feature Implementation', yaxis_title='<b>← Dissatisfaction ... Satisfaction →</b><br>Clinician Response', legend=dict(orientation='v', y=0.99, x=0.01, yanchor='top', xanchor='left', bgcolor='rgba(255,255,255,0.8)'))
    return fig

def plot_risk_signal_clusters(df_clustered: pd.DataFrame) -> go.Figure:
    fig = px.scatter(df_clustered, x='Temp_C', y='Pressure_psi', color='cluster',
                     title="<b>Risk Signal Clustering via DBSCAN</b>",
                     labels={'cluster': 'Process Cluster', 'Temp_C': 'Temperature (°C)', 'Pressure_psi': 'Pressure (psi)'},
                     color_discrete_map={'0': COLORS['primary'], '1': COLORS['secondary'], 'Outlier/Anomaly': COLORS['danger']},
                     hover_data=['Source'])
    fig.update_traces(marker=dict(size=10, line=dict(width=1, color='DarkSlateGrey')))
    fig.update_layout(legend_title_text='Signal Cluster')
    return fig

def plot_gage_rr_pareto(df_gage: pd.DataFrame) -> go.Figure:
    # FIX: Corrected function signature and implemented a proper Pareto Chart.
    df_sorted = df_gage.sort_values('Contribution (%)', ascending=False)
    df_sorted['Cumulative'] = df_sorted['Contribution (%)'].cumsum()
    
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Bar(x=df_sorted['Source of Variation'], y=df_sorted['Contribution (%)'], name='Contribution', marker_color=COLORS['primary']), secondary_y=False)
    fig.add_trace(go.Scatter(x=df_sorted['Source of Variation'], y=df_sorted['Cumulative'], name='Cumulative', mode='lines+markers', line=dict(color=COLORS['accent'])), secondary_y=True)
    fig.update_layout(title="<b>Gage R&R:</b> Sources of Measurement Variation", xaxis_title="Source of Variation")
    fig.update_yaxes(title_text="Contribution (%)", secondary_y=False)
    fig.update_yaxes(title_text="Cumulative (%)", secondary_y=True, range=[0, 101])
    return fig
...
def plot_capability_analysis_pro(data: np.ndarray, lsl: float, usl: float) -> Tuple[go.Figure, float, float]:
    ...
    x_range = np.linspace(min(lsl, data.min()) - std, max(usl, data.max()) + std, 400); y_pdf = norm.pdf(x_range, mean, std)
    ...
    (osm, osr), (slope, intercept, r) = probplot(data, dist="norm"); fig.add_trace(go.Scatter(x=osm, y=osr, mode='markers', name='Data', marker=dict(color=COLORS['primary'])), row=1, col=2); fig.add_trace(go.Scatter(x=osm, y=slope*osm + intercept, mode='lines', name='Fit', line=dict(color=COLORS['danger'])), row=1, col=2)
    verdict = "Capable" if cpk >= 1.33 else ("Marginal" if cpk >= 1.0 else "Not Capable"); verdict_color = COLORS['success'] if verdict == 'Capable' else (COLORS['warning'] if verdict == 'Marginal' else COLORS['danger'])
    fig.update_layout(title=f"<b>Process Capability Report — Verdict: <span style='color:{verdict_color};'>{verdict}</span></b>", xaxis_title="Measurement", yaxis_title="Density", showlegend=False, xaxis2_title="Theoretical Quantiles", yaxis2_title="Sample Quantiles")
    metrics_text = (f"<b>Process Metrics</b><br>Mean: {mean:.3f}<br>Std Dev: {std:.3f}<br>Cp: <b>{cp:.3f}</b><br>Cpk: <b style='color:{verdict_color};'>{cpk:.3f}</b><br>Target Cpk: 1.33")
    fig.add_annotation(text=metrics_text, align='left', showarrow=False, xref='paper', yref='paper', x=1.0, y=1.0, bgcolor="rgba(255,255,255,0.8)", bordercolor=COLORS['dark_gray'], borderwidth=1)
    return fig, cp, cpk

def plot_fishbone_plotly() -> go.Figure:
    nodes = {
        'effect': {'x': 8, 'y': 5, 'text': 'EFFECT<br>Low Library Yield', 'color': COLORS['danger'], 'font_color': 'white'},
        'reagents': {'x': 3, 'y': 8, 'text': 'Reagents', 'color': COLORS['primary'], 'font_color': 'white'},
        'c1': {'x': 1, 'y': 9, 'text': 'Enzyme Inactivity', 'color': COLORS['light_gray']},
        'equipment': {'x': 5, 'y': 8, 'text': 'Equipment', 'color': COLORS['primary'], 'font_color': 'white'},
        'c2': {'x': 3, 'y': 9, 'text': 'Pipette out of cal', 'color': COLORS['light_gray']},
        'method': {'x': 7, 'y': 8, 'text': 'Method', 'color': COLORS['primary'], 'font_color': 'white'},
        'c3': {'x': 5, 'y': 9, 'text': 'Wrong incubation time', 'color': COLORS['light_gray']},
        'technician': {'x': 3, 'y': 2, 'text': 'Technician', 'color': COLORS['primary'], 'font_color': 'white'},
        'c4': {'x': 1, 'y': 1, 'text': 'Inconsistent pipetting', 'color': COLORS['light_gray']},
        'sample': {'x': 5, 'y': 2, 'text': 'Sample', 'color': COLORS['primary'], 'font_color': 'white'},
        'c5': {'x': 3, 'y': 1, 'text': 'Low DNA input', 'color': COLORS['light_gray']},
        'environment': {'x': 7, 'y': 2, 'text': 'Environment', 'color': COLORS['primary'], 'font_color': 'white'},
        'c6': {'x': 5, 'y': 1, 'text': 'High humidity', 'color': COLORS['light_gray']},
    }
    edges = [('effect', 'reagents'), ('reagents', 'c1'), ('effect', 'equipment'), ('equipment', 'c2'),
             ('effect', 'method'), ('method', 'c3'), ('effect', 'technician'), ('technician', 'c4'),
             ('effect', 'sample'), ('sample', 'c5'), ('effect', 'environment'), ('environment', 'c6')]
...
def plot_pareto_chart(df_pareto: pd.DataFrame) -> go.Figure:
    df_sorted = df_pareto.sort_values('Frequency', ascending=False); df_sorted['Cumulative Percentage'] = df_sorted['Frequency'].cumsum() / df_sorted['Frequency'].sum() * 100
    fig = make_subplots(specs=[[{"secondary_y": True}]]); fig.add_trace(go.Bar(x=df_sorted['QC_Failure_Mode'], y=df_sorted['Frequency'], name='Failure Count', marker_color=COLORS['primary']), secondary_y=False); fig.add_trace(go.Scatter(x=df_sorted['QC_Failure_Mode'], y=df_sorted['Cumulative Percentage'], name='Cumulative %', mode='lines+markers', line_color=COLORS['accent'], line_width=3), secondary_y=True); fig.add_hline(y=80, line=dict(color=COLORS['dark_gray'], dash='dot', width=2), secondary_y=True, annotation_text="80% Line", annotation_position="bottom right"); fig.update_layout(title_text="<b>Pareto Chart:</b> Identifying the 'Vital Few' Failure Modes", xaxis_title="QC Failure Mode", bargap=0.4); fig.update_yaxes(title_text="<b>Frequency</b>", secondary_y=False); fig.update_yaxes(title_text="<b>Cumulative Percentage</b>", secondary_y=True, range=[0, 101], ticksuffix='%'); return fig

def plot_regression_comparison(model_results: dict) -> go.Figure:
    X, y = model_results['X'], model_results['y']; primary_factor = 'Annealing_Temp'; sort_idx = X[primary_factor].argsort()
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=X[primary_factor].iloc[sort_idx], y=y.iloc[sort_idx], mode='markers', name='Actual Data', marker=dict(color=COLORS['dark_gray'], opacity=0.5)))
    fig.add_trace(go.Scatter(x=X[primary_factor].iloc[sort_idx], y=model_results['linear_predictions'][sort_idx], mode='lines', name=f"Linear Model (R²={model_results['linear_r2']:.2f})", line=dict(color=COLORS['primary'], width=3)))
    fig.add_trace(go.Scatter(x=X[primary_factor].iloc[sort_idx], y=model_results['rf_predictions'][sort_idx], mode='lines', name=f"Random Forest (OOB R²={model_results['rf_oob_r2']:.2f})", line=dict(color=COLORS['secondary'], width=3, dash='dot')))
    fig.update_layout(title_text="<b>Regression:</b> Modeling Assay Performance", xaxis_title=f"Primary Factor: {primary_factor.replace('_', ' ')} (°C)", yaxis_title="On-Target Rate (%)")
    return fig

def plot_shap_summary(shap_explanation: shap.Explanation) -> go.Figure:
    shap_values = shap_explanation.values; feature_names = shap_explanation.feature_names
    mean_abs_shap = np.abs(shap_values).mean(axis=0); importance_df = pd.DataFrame({'feature': feature_names, 'importance': mean_abs_shap}).sort_values('importance')
    fig = make_subplots(rows=1, cols=2, column_widths=[0.6, 0.4], subplot_titles=("<b>Feature Impact (Beeswarm)</b>", "<b>Global Importance (Mean |SHAP|)</b>"), horizontal_spacing=0.15)
    for i, feature in enumerate(importance_df['feature']):
        feature_idx = feature_names.index(feature)
        fig.add_trace(go.Scatter(x=shap_values[:, feature_idx], y=np.full(shap_values.shape[0], i), mode='markers', marker=dict(color=shap_explanation.data[:, feature_idx], colorscale='RdBu', showscale=True, colorbar=dict(title='Feature Value', x=0.58, y=0.5, len=0.8), symbol='circle-open', line_width=1), customdata=shap_explanation.data[:, feature_idx], hovertemplate=f"<b>{feature}</b><br>SHAP value: %{{x:.3f}}<br>Feature value: %{{customdata:.3f}}", name=feature), row=1, col=1)
    fig.add_trace(go.Bar(x=importance_df['importance'], y=importance_df['feature'], orientation='h', marker_color=COLORS['secondary']), row=1, col=2)
    fig.update_layout(title="<b>SHAP Summary: Explaining the Model's Predictions</b>", showlegend=False, yaxis=dict(tickmode='array', tickvals=list(range(len(importance_df))), ticktext=importance_df['feature'], showgrid=False), xaxis=dict(title="SHAP Value (Impact on Output)", zeroline=True), yaxis2=dict(showticklabels=False), xaxis2=dict(title="Mean Absolute SHAP Value"))
    return fig

def plot_fault_tree_plotly() -> go.Figure:
    nodes = {'top': {'x': 2, 'y': 4.5, 'text': 'TOP EVENT<br>False Negative Result', 'color': COLORS['danger'], 'font_color': 'white'}, 'or1': {'x': 2, 'y': 3.5, 'text': 'OR Gate', 'color': COLORS['dark_gray'], 'font_color': 'white'}, 'and1': {'x': 0.5, 'y': 2, 'text': 'AND Gate', 'color': COLORS['dark_gray'], 'font_color': 'white'}, 'assay': {'x': 3.5, 'y': 2.5, 'text': 'Assay Failure', 'color': COLORS['primary'], 'font_color': 'white'}, 'reagent': {'x': 0, 'y': 1, 'text': 'Reagent Degraded<br>P=0.01', 'color': COLORS['secondary'], 'font_color': 'white'}, 'storage': {'x': 1, 'y': 1, 'text': 'Improper Storage<br>P=0.05', 'color': COLORS['secondary'], 'font_color': 'white'}, 'sample': {'x': 3.5, 'y': 1.5, 'text': 'Low DNA Input<br>P=0.02', 'color': COLORS['secondary'], 'font_color': 'white'}}
...
def plot_5whys_diagram() -> go.Figure:
    ...
    for i, (level, text, color) in enumerate(steps):
        nodes[f's{i}'] = {'x': 1, 'y': 5 - i, 'text': f'{level}<br>{text}', 'color': color, 'font_color': 'white'}
...
def plot_nlp_on_capa_logs(df_topics: pd.DataFrame) -> go.Figure:
    # FIX: Explicitly provided x and y to resolve ValueError.
    fig = px.bar(df_topics, x='Frequency', y='Topic', orientation='h',
                 labels={'y': 'Identified Failure Theme', 'x': 'Frequency'},
                 title="<b>NLP Topic Modeling on CAPA & Deviation Logs</b>",
                 color='Topic',
                 color_discrete_map={"Reagent/Storage Issue": COLORS['primary'], "Contamination": COLORS['accent'], "Hardware Failure": COLORS['danger'], "Human Error": COLORS['warning']})
...
def plot_rsm_contour(df_rsm: pd.DataFrame) -> go.Figure:
    fig = go.Figure(data=go.Contour(z=df_rsm['Yield'], x=df_rsm['Temperature'], y=df_rsm['Concentration'], colorscale='Viridis', contours=dict(coloring='heatmap', showlabels=True, labelfont=dict(size=12, color='white'))))
    max_yield_point = df_rsm.loc[df_rsm['Yield'].idxmax()]; fig.add_trace(go.Scatter(x=[max_yield_point['Temperature']], y=[max_yield_point['Concentration']], mode='markers', marker=dict(color=COLORS['danger'], size=15, symbol='star'), name='Optimal Point'))
    fig.update_layout(title="<b>Response Surface Methodology (RSM)</b>", xaxis_title="Temperature (°C)", yaxis_title="Enzyme Concentration", showlegend=False); return fig
...
def plot_ewma_chart(df_control: pd.DataFrame, lambda_val: float = 0.2) -> go.Figure:
    data = df_control['Yield_ng']; mean, std_dev = data.iloc[:75].mean(), data.iloc[:75].std(ddof=1)
    ewma = data.ewm(span=(2/lambda_val) - 1, adjust=False).mean(); limit_factor = 3 * std_dev * np.sqrt(lambda_val / (2 - lambda_val)); ucl, lcl = mean + limit_factor, mean - limit_factor
...
def plot_cusum_chart(df_control: pd.DataFrame, k: float = 0.5, h: float = 5.0) -> go.Figure:
    data = df_control['Yield_ng']; mean, std_dev = data.iloc[:75].mean(), data.iloc[:75].std(ddof=1); z = (data - mean) / std_dev
    cusum_pos = np.zeros(len(z)); cusum_neg = np.zeros(len(z));
    ...
    fig.update_layout(title=f'<b>CUSUM Chart:</b> Accumulating Process Shift Info', xaxis_title='Batch ID', yaxis_title='Cumulative Sum'); return fig

def plot_hotelling_t2_chart(df_hotelling: pd.DataFrame) -> go.Figure:
    in_control_data = df_hotelling.iloc[:80]; mean_vec, cov_matrix = in_control_data.mean(), in_control_data.cov(); inv_cov_matrix = np.linalg.inv(cov_matrix); t_squared = [(row - mean_vec).T @ inv_cov_matrix @ (row - mean_vec) for _, row in df_hotelling.iterrows()]; p, m = len(mean_vec), len(in_control_data); ucl = (p * (m - 1) * (m + 1)) / (m * m - m * p) * f_dist.ppf(0.99, p, m - p)
    df_hotelling['T_Squared'] = t_squared; out_of_control_points = df_hotelling[df_hotelling['T_Squared'] > ucl]; fig = make_subplots(rows=2, cols=1, shared_xaxes=False, subplot_titles=("<b>When did it fail?</b> (T² Control Chart)", "<b>Why did it fail?</b> (Process Variable Scatter)"), vertical_spacing=0.15)
...
def plot_adverse_event_clusters(df_clustered: pd.DataFrame) -> go.Figure:
    cluster_names = {0: "Neurological (Headache, Dizziness)", 1: "Allergic / Skin Reaction", 2: "Systemic (Liver, Anaphylaxis)", 3: "Gastrointestinal / Injection Site"}; df_clustered['cluster_name'] = df_clustered['cluster'].map(cluster_names).fillna("Other")
    fig = px.scatter(df_clustered, x='x_pca', y='y_pca', color='cluster_name', hover_data=['description'], title="<b>ML Clustering of Adverse Event Narratives</b>", labels={'color': 'Event Cluster'})
    fig.update_layout(xaxis_title="PCA Component 1", yaxis_title="PCA Component 2", xaxis=dict(showticklabels=False, zeroline=False), yaxis=dict(showticklabels=False, zeroline=False)); return fig

def plot_pccp_monitoring(df_pccp: pd.DataFrame) -> go.Figure:
    fig = go.Figure(); fig.add_trace(go.Scatter(x=df_pccp['Deployment_Day'], y=df_pccp['Model_AUC'], mode='lines', name='Model Performance (AUC)', line=dict(color=COLORS['primary']))); fig.add_hline(y=0.90, line=dict(color=COLORS['danger'], dash='dash'), name='Performance Threshold'); fig.add_vrect(x0=70, x1=100, fillcolor=hex_to_rgba(COLORS['warning'], 0.2), line_width=0, name="Performance Degradation")
    fig.add_annotation(x=85, y=0.87, text="<b>Retraining & Revalidation<br>Triggered per PCCP</b>", showarrow=True, arrowhead=1, ax=0, ay=-40, bgcolor="rgba(255,255,255,0.8)"); fig.update_layout(title="<b>PCCP Monitoring for an AI/ML Device (SaMD)</b>", xaxis_title="Days Since Deployment", yaxis_title="Model Area Under Curve (AUC)", legend=dict(x=0.01, y=0.01, yanchor='bottom')); return fig
...
def plot_comparison_radar() -> go.Figure:
    ...
    fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 5.5])), title="<b>Strengths Profile:</b> Classical vs. ML for Biotech R&D", legend=dict(yanchor="bottom", y=-0.2, xanchor="center", x=0.5)); return fig
...
def plot_verdict_barchart() -> go.Figure:
    ...
    fig.update_layout(xaxis=dict(tickvals=[-1, 1], ticktext=['<b>Winner: Classical Stats</b>', '<b>Winner: Machine Learning</b>'], tickfont=dict(size=14), range=[-1.5, 1.5]), yaxis_title=None, bargap=0.4, showlegend=False); return fig

def plot_synergy_diagram() -> go.Figure:
    fig = go.Figure()
    fig.update_layout(height=400, showlegend=False, xaxis=dict(visible=False, range=[0, 3.2]), yaxis=dict(visible=False, range=[0,2]), margin=dict(t=80, b=10, l=10, r=10))
    fig.add_shape(type="circle", x0=0, y0=0, x1=2, y1=2, line_color=COLORS['primary'], fillcolor=hex_to_rgba(COLORS['primary'], 0.6))
    fig.add_shape(type="circle", x0=1.2, y0=0, x1=3.2, y1=2, line_color=COLORS['secondary'], fillcolor=hex_to_rgba(COLORS['secondary'], 0.6))
    ...
    fig.update_layout(title="<b>The Hybrid Philosophy:</b> Combining Strengths"); return fig
